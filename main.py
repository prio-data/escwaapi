...
app = FastAPI()
vRuns = DBModel.Runs()
dirty_full_model_list = asyncio.run(vRuns.init_and_fetch(), debug=True)

AllModels = Enum('AllModels', {i: i for i in dirty_full_model_list})
...
dateQuery = Query(default=None, pattern=r'^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])$',
                  title="date",
...
async def get_metadata(run: AvailableRuns):
    ...
    codebook = await cur_run.codebook_file
...
async def get_run(run: AvailableRuns, loa: AvailableLoa, request: Request,
    ...
    if data:
        ...
        await data_fetcher.register_where_iso(iso)
        await data_fetcher.register_where_gwno(gwno)
        ...
        dataset = await data_fetcher.fetch(page=page)
        row_count, page_count = await data_fetcher.total_counts()
...
async def get_run(run: AvailableRuns, loa: AvailableLoa, tv: AvailableTypeOfViolence, request: Request,
    ...
    if data:
        ...
        data_fetcher.register_where_bbox_coord(corner1_lat=lat_ne, corner1_lon=lon_ne,
                                               corner2_lat=lat_sw, corner2_lon=lon_sw)
        if is_escwa:
            await data_fetcher.register_where_escwa()
